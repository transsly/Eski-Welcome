...
console.log(`Discow / Welcome's | ${mesaj} | Bot : ${client.user.tag} | Bot ID : ${client.user.id} | Bot Sırası : 2 | Tarih : [${moment(new Date()).format("YYYY-MM-DD HH:mm:ss")}]`);
};

function discowhata(hata) {
console.error(`Discow / Welcome's | Botta Bir Hata Oluştu. | Bot Sırası : 2 | Tarih : [${moment(new Date()).format("YYYY-MM-DD HH:mm:ss")}]
Hata: ${hata}`);
};
...
client.login(process.env.BOTTOKEN2).then(x => discowlog(`Bot Başarıyla Giriş Yaptı`)).catch(err => discowhata(`Bot Giriş Yaparen Bir Hata Oluştu.
Hata : [${err.message}]`))
