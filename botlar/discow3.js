...
console.log(`Discow / Welcome's | ${mesaj} | Bot : ${client.user.tag} | Bot ID : ${client.user.id} | Bot Sırası : 3 | Tarih : [${moment(new Date()).format("YYYY-MM-DD HH:mm:ss")}]`);
};

function discowhata(hata) {
console.error(`Discow / Welcome's | Botta Bir Hata Oluştu. | Bot Sırası : 3 | Tarih : [${moment(new Date()).format("YYYY-MM-DD HH:mm:ss")}]
Hata: ${hata}`);
};
...
const Channel = client.channels.cache.get(ayarlar.odalar.welcome3)
...
client.login(process.env.BOTTOKEN3).then(x => discowlog(`Bot Başarıyla Giriş Yaptı`)).catch(err => discowhata(`Bot Giriş Yaparen Bir Hata Oluştu.
Hata : [${err.message}]`))
